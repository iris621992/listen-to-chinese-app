...
test("prefetch RSC is excluded from initial-route attribution only", () => {
  ...
  assert.equal(
    browserResponseBelongsToInitialRoute(rscResponse, "non-prefetch/unknown"),
    true,
  );
  assert.throws(
...
test("implementation stays public-safe and bound to actual production surfaces", async () => {
  ...
  assert.match(
    browserSource,
    /attributeRouteBytes\(\s*client,\s*initialRouteAttributionEntries/,
  );
  assert.doesNotMatch(
    browserSource,
    /attributeRouteBytes\(client, attributionEntries\)/,
  );
